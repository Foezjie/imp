...
class RelationAttribute(Attribute):
    ...
    def set_attribute(self, instance, value, double = True):
        ...
        if self.low == 1 and self.high == 1:
            ...
            if self.name not in instance._attributes or \
                    instance._attributes[self.name] is None or \
                    isinstance(instance._attributes[self.name], Unset):
                current_value = Variable(QList())
