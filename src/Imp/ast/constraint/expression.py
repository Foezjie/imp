...
from Imp.ast.statements.call import FunctionCall
...
class OpMetaClass(ABCMeta):
    ...
    def __init__(mcs, name, bases, attr_dict):
        ...
        if attribute in attr_dict:
            Operator.register_operator(attr_dict[attribute], mcs)
        super(OpMetaClass, mcs).__init__(name, bases, attr_dict)

class Operator(CallStatement, metaclass=OpMetaClass):
    """ 
        This class is an abstract base class for all operators that can be used
        in expressions  
    """
    ...
    def get_operator_class(cls, oper):
        """
            Get the class that implements the given operator. Returns none of 
    ...
    def get_variables(self):
        """
            Get the variables that need a value. If one of the operands is 
            an operator itself its variables will be merged.
        """
        variables = set()
        for arg in self._arguments:
            if hasattr(arg, "eval"):
                ...
                # a function call
                variables = variables.union(arg.arguments)
    ...
    def eval(self, variables = None, var_list = None, state = None):
        """ 
            The call method implements the operator and returns the result of 
            the operator
            
            @param variables: A dictionary that contains the values for the 
        ...
        for arg in self._arguments:
            if hasattr(arg, "eval"): # we have an operator
                value = arg.eval(variables, var_list)
                ...
                if isinstance(arg, Reference) or isinstance(arg, AttributeVariable):
                    if positional and len(positional_list) > 0:
                        ...
                        if not hasattr(arg, "name") or\
                                                arg.name not in variables:
                ...
                if hasattr(arg, "value"):
                    ...
                    for fn_arg in arg.arguments:
                        fn_arg = str(fn_arg)
                        if fn_arg in variables:
                            fnc_arglist.append(variables[fn_arg])
                        else:
...
class LessThan(BinaryOperator):
    ...
    def _bin_op(self, arg1, arg2):
        """
            @see Operator#_op
        """
        if not isinstance(arg1[0], (int, float)) or\
                not isinstance(arg2[0],  (int, float)): 
...
class Is(BinaryOperator):
    """ 
        Operator that checks if a value "is a" in an enum.
    """
    __op = "is"
    
    def __init__(self, op1, op2):
        BinaryOperator.__init__(self, "is")
        self._add_operand(0, op1)
        self._add_operand(1, op2)
    ...
    def _bin_op(self, arg1, arg2):
        """
            @see Operator#_op
        """
        if not arg1[1].validate(arg2[0]):
            return False
        
        if arg1[0] == arg2[0]:
            return True
        
        return arg1[1].is_child_of(arg1[0], arg2[0])
