...
class Entity(Type, QList):
    """
        This class models a defined entity in the domain model of the 
        configuration model.
    ...
    def add_attribute(self, attribute):
        """
            Add an attribute to this entity. The attribute should not 
            exist yet.
        """
            ...
            for parent in self.parent_entities:
                for attr in parent.attributes.keys():
                    if attr not in attributes:
                        ...
                        raise Exception("Hiding attributes with inheritance " +
                            "is not allowed. %s is already defined" % attr)
                    
                cls = parent.get_class_type()
                parents.append(cls)
            
            # generate the class with voodoo
            self.__cls_type = EntityTypeMeta.__new__(EntityTypeMeta, "", \
                tuple(parents), {"__definition__" : self})
    ...
    def validate(self, value):
        ...
        if not hasattr(value.__class__, "__definition__"):
            raise ValueError("Invalid class type %s, should be %s" %
        ...
        for index_attributes in self._index_def:
            for attribute in index_attributes:
                if attribute not in attributes:
                    raise Exception("Index with attributes %s defined on entity %s is invalid. Attribute %s does not exist in this entity." % 
