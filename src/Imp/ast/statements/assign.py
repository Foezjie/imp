...
from Imp.ast.variables import Variable, AttributeVariable, Reference
...
class SetAttribute(GetAttribute):
    ...
    def actions(self, state):
        ...
        if isinstance(instance_ref.value, QList):
            raise Exception("Unable to set attribute on a list.")

        if isinstance(value_ref.value, QList):
            actions = []
            list_refs = []
    ...
    def __repr__(self):
        return "%s.%s = %s" % (self.instance_name, \
