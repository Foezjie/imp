...
class FunctionCall(CallStatement):
    ...
    def __repr__(self):
        ...
        return "Function(%s: %s)" % (self.name, \
                            ", ".join([repr(x) for x in self.arguments]))
...
class Expression(CallStatement):
        ...
        for name, ref in refs:
            if hasattr(ref, "name"):
                if ref.name in self.arguments and len(ref.namespace) == 0:
                    # keep the ref for us
                    self._argument_map[name] = \
                        (self.arguments.index(ref.name), ref)
    ...
    def new_statements(self, state):
        """
            Hack to register the expression variables -> we need the state 
            for this
        """
        exp_state = ExpressionState(state, self.expression, self.arguments, \
                                    self._argument_map)
