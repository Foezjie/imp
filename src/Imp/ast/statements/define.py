...
from Imp.ast.variables import Variable, Reference
from Imp.ast.type import ConstraintType, Type
from Imp.ast.attribute import Attribute, RelationAttribute
from Imp.ast.entity import Implementation, Entity, Default,\
    Implement
from Imp.ast.constraint.expression import Equals
...
class DefineEntity(DefinitionStatement):
    ...
    def evaluate(self, state, local_scope):
        ...
        for attribute in self.attributes:
            ...
            if not isinstance(attr_type, (Type, type, Enum)):
        ...
        for parent in self.parents:
            ...
            for attr_name in parent_type.attributes.keys():
                if attr_name not in add_attributes:
                    ...
                    raise Exception("Hiding attributes with inheritance is " +
                            "not allowed. %s is already defined" % attr_name)
...
class DefineTypeConstraint(DefinitionStatement):
    ...
    def set_expression(self, expression):
        ...
        if not contains_var:
            raise Exception("typedef expressions should reference the" +\
                " self variable")
...
class DefineTypeDefault(DefinitionStatement):
    """ 
        Define a new entity that is based on an existing entity and 
...
class DefineRelation(DefinitionStatement):
    ...
    def evaluate(self, state, local_scope):
        ...
        if left.get_attribute(self.right[1]) is not None:
            raise Exception(("Attribute name %s is already defined in %s, " +
                "unable to define relationship") % (self.right[1], left.name))
        ...
        if right.get_attribute(self.left[1]) is not None:
            raise Exception(("Attribute name %s is already defined in %s, " +
                "unable to define relationship") % (self.left[1], right.name))
...
class DefineEnum(DefinitionStatement):
    """
        Define an enumeration
    """
    def __init__(self, enum_type, parent, items):
        DefinitionStatement.__init__(self)
    ...
    def __repr__(self):
        return "%s(^%s %s)" % (self.enum_type, self.parent, " ".join(self.items))
