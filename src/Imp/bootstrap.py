...
def deploy_key(ip, key):
        run("/usr/bin/ssh", ARGS + ["ec2-user@" + ip, 'echo "%s" > /tmp/key.pub; sudo mv /tmp/key.pub /root/.ssh/authorized_keys; sudo chown root:root /root/.ssh/authorized_keys; sudo chmod 600 /root/.ssh/authorized_keys' % key])
