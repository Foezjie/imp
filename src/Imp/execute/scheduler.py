...
from Imp.ast.statements import DefinitionStatement, CallStatement, \
    DynamicStatement
from .state import State, DynamicState
from Imp.execute.scope import Scope
from Imp.execute.util import EntityType, Unset
from Imp.execute.proxy import UnsetException
    ...
    def print_unresolved(self, stmt):
        ...
        for ref in stmt._refs.values():
            if not ref.is_available(scope):
                sys.stderr.write("\t%s not available in scope %s (%s)\n" %
                    (ref, scope, " > ".join(scope.path())))
    ...
    def check_unset_attributes(self, obj):
        ...
        for attr in attributes:
            ...
            if isinstance(value, Unset):
                self.show_exception(obj.__statement__, 
                    "Attribute '%s' of object %s is not set." % (attr, obj))
