...
#from Imp.export import PickleTypeHelper, ObjId

# pylint: disable-msg=E1101, R0903, W0212
from Imp.ast.constraint.query import QList

import sys, imp, functools
...
class Unknown(object):
    """
        An instance of this class is used to indicate that this value can not 
        be determined yet.
...
class EntityTypeMeta(type):
    """
        A metaclass that transform an object of EntityType to a subclass that
        is based on the entity definition 
    """
...
class EntityType(object, metaclass=EntityTypeMeta):
    ...
    def __setattr__(self, name, value):
        if name == "_attributes" or name == "_childeren" or \
                self.__class__.__definition__.has_attribute(name):
