...
from Imp.ast.type import TYPES, Bool, String, Number

from . import ImpParser
from . import ImpLexer

from Imp.ast.statements import Statement
from Imp.ast.statements.define import DefineEntity, \
    DefineTypeDefault, DefineImplementation, DefineTypeConstraint, DefineImplement, \
    DefineRelation, DefineEnum, DefineIndex
from Imp.ast.statements.assign import SetAttribute, Assign,\
    CreateList, IndexLookup, StringFormat
from Imp.ast.statements.call import FunctionCall, Expression
from Imp.ast.statements.generator import Constructor, Import, For

from Imp.ast.constraint.query import QList
from Imp.ast.constraint.expression import Operator 
from Imp.ast.constraint.expression import Regex as RegexOp

from Imp.ast.variables import Variable, Reference, AttributeVariable
import pickle
...
class Parser(object):
    ...
    @action(ImpParser.ENUM)
    def create_enum(self, node):
        """
            Create an enumeration
        """
        items = [self._handle_node(x) for x in node.children[1].children]
        
        if node.children[1].type != ImpParser.STRING and node.children[1].text == "root":
            parent = None
            
        else:
            parent = self._handle_node(node.children[1])
        
        enum = DefineEnum(self._handle_node(node.children[0]), parent, items)
        
        return enum
    
    @action(ImpParser.REF)
    ...
    @action(ImpParser.DEF_IMPLEMENT)
    ...
    @action(ImpParser.DEF_IMPLEMENTATION)
    ...
    def create_hash_lookup(self, node):
        ...
        params = [(x.children[0].text, self._handle_node(x.children[1])) 
                        for x in node.children[1].children]
        return IndexLookup(class_ref, params)
    
    @action(ImpParser.DEF_RELATION)
    ...
    @action(ImpParser.LAMBDA)
    def create_lambda(self, node):
        """
            Create a lambda expression
        """
        arg = str(node.children[0])
        if node.children[1].type == ImpParser.ORPHAN:
    ...
    def _handle_node(self, node):
        ...
        if create_func is None:
            raise Exception("Unable to handle statement %s (%d)" % 
                        (node, node.type))
    ...
    def check_cache(self, filename):
        """
            Check if a compiled file exists
        """
        cache_file = filename + "c"
        ...
        if file_stat.st_mtime >= cache_stat.st_mtime:
            return None
    ...
    def cache(self, filename, statements):
        """
            Cache the given file
        """
        cache_file = filename + "c"
        with open(cache_file, "w+") as fd:
            pickle.dump(statements, fd)
    
    def parse(self, namespace, filename):
        ast = self.check_cache(filename)
        ...
        lexer = ImpLexer.ImpLexer(char_stream)
        tokens = antlr3.CommonTokenStream(lexer)
        parser = ImpParser.ImpParser(tokens)
