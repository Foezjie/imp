...
class PluginMeta(type):
    ...
    def add_function(mcs, plugin_class):
        ...
        if not (ns_parts[0] == "Imp" and ns_parts[1] == "plugins"):
...
class Plugin(object, metaclass = PluginMeta):
    ...
    def check_args(self, args):
        ...
        for i in range(len(args)):
            if self.arguments[i][0] is not None and \
                    not self._is_instance(args[i], self.arguments[i][1]):
    ...
    def check_requirements(self):
        """
            Check if the plugin has all it requires
        """
        if "bin" in self.opts and self.opts["bin"] is not None:
            for bin in self.opts["bin"]:
                p = subprocess.Popen(["bash", "-c", "type -p %s" % bin], stdout = subprocess.PIPE)
                result = p.communicate()
                
                if len(result[0]) == 0:
                    print("%s requires %s to be available in $PATH" % (self.__function_name__, bin))
...
def plugin(function = None, bin = None):
    """
        Python 3 decorator to register functions with Imp
    """
    def curry_name(name = None, bin = None):
        """
    ...
    if function is None and bin is not None:
        return curry_name(bin = bin)
